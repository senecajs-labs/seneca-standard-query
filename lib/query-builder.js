  ...
  function parseExpression (current_name, current_value) {
    if (current_name === 'or$') {
      ...
      if (current_value === null) {
        ...
        params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase(current_name)) + sTypes.escape + op + preparedStatements(sTypes.name, values.length))
      }
        ...
        params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase(current_name)) + sTypes.escape + '=' + preparedStatements(sTypes.name, values.length))
      }
    }
    return {}
  }
...
function preparedStatements (dbType, paramCount) {
...
function savestm (ent, sTypes) {
  ...
  fields.forEach(function (field) {
    ...
    params.push(preparedStatements(sTypes.name, ++cnt))
  })
...
function updatestm (ent, sTypes) {
  ...
  fields.forEach(function (field) {
    ...
    if (!_.isUndefined(entp[field])) {
      values.push(entp[field])
      params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase(field)) + sTypes.escape + '=' + preparedStatements(sTypes.name, ++cnt))
    }
  })
...
function deletestm (qent, q, sTypes) {
  ...
  if (!_.isEmpty(w) && w.params.length > 0) {
    for (var i in w.params) {
      ...
      params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase(param)) + sTypes.escape + '=' + preparedStatements(sTypes.name, ++cnt))
...
function selectstmOr (qent, q, sTypes) {
  ...
  if (!_.isEmpty(w) && w.params.length > 0) {
    w.params.forEach(function (param) {
      params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase('id')) + sTypes.escape + '=' + preparedStatements(sTypes.name, ++cnt))
    })
