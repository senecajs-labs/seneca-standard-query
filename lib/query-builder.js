...
var snakeCamel = snakeToCamelCase
var camelSnake = camelCaseToSnake

module.exports = function (opts) {
  if (opts.snakeToCamelCase) {
    snakeCamel = opts.snakeToCamelCase
  }
  if (opts.camelCaseToSnake) {
    camelSnake = opts.camelCaseToSnake
  }
}
...
function parseExpression (entp, sTypes, current_name, current_value, params, values) {
  ...
  if (current_value === null) {
    ...
    params.push(sTypes.escape + RelationalStore.escapeStr(camelSnake(current_name)) + sTypes.escape + ' IS NULL')
    return result
  }
  ...
  if (current_value instanceof RegExp) {
    ...
    params.push(sTypes.escape + RelationalStore.escapeStr(camelSnake(current_name)) + sTypes.escape + op + RelationalStore.preparedStatements(sTypes.name, values.length))
    return result
  }
  ...
  if (_.isObject(current_value)) {
    ...
    params.push(sTypes.escape + RelationalStore.escapeStr(camelSnake(current_name)) + sTypes.escape + '=' + RelationalStore.preparedStatements(sTypes.name, values.length))
    return result
  }
...
var jsonSupport = function (sTypes) {
...
function savestm (ent, sTypes) {
  ...
  fields.forEach(function (field) {
    escapedFields.push(sTypes.escape + RelationalStore.escapeStr(camelSnake(field)) + sTypes.escape)
...
function updatestm (ent, sTypes) {
  ...
  fields.forEach(function (field) {
    ...
    if (!_.isUndefined(entp[field])) {
      values.push(entp[field])
      params.push(sTypes.escape + RelationalStore.escapeStr(camelSnake(field)) + sTypes.escape + '=' + RelationalStore.preparedStatements(sTypes.name, ++cnt))
    }
  })
...
function deletestm (qent, q, sTypes) {
  ...
  if (!_.isEmpty(w) && w.params.length > 0) {
    for (var i in w.params) {
      ...
      params.push(sTypes.escape + RelationalStore.escapeStr(camelSnake(param)) + sTypes.escape + '=' + RelationalStore.preparedStatements(sTypes.name, ++cnt))
...
function filterStatement (qent, q) {
  ...
  if (!_.isEmpty(w) && w.params.length > 0) {
    w.params.forEach(function (param) {
      params.push('"' + RelationalStore.escapeStr(camelSnake(param)) + '"=$' + (++cnt))
    })
...
function selectstmOr (qent, q, sTypes) {
  ...
  if (!_.isEmpty(w) && w.params.length > 0) {
    w.params.forEach(function (param) {
      params.push(sTypes.escape + RelationalStore.escapeStr(camelSnake('id')) + sTypes.escape + '=' + RelationalStore.preparedStatements(sTypes.name, ++cnt))
    })
...
function metaquery (qent, q) {
  ...
  if (q.sort$) {
    ...
    mq.params.push('ORDER BY ' + camelSnake(sf) + ' ' + sd)
  }
...
function snakeToCamelCase (attr) {
  return attr
}

function camelCaseToSnake (attr) {
  return attr
}
...
module.exports.selectstmCustom = selectstmCustom
...
module.exports.jsonSupport = jsonSupport
module.exports.whereargs = whereargs
module.exports.metaquery = metaquery
module.exports.buildQueryFromExpressionCustom = buildQueryFromExpressionCustom
module.exports.parseExpression = parseExpression
module.exports.snakeToCamelCase = snakeCamel
module.exports.camelCaseToSnake = camelSnake
module.exports.makeent = RelationalStore.makeent
