...
const mySQLStore = 'mysql-store'
const postgresStore = 'postgresql-store'
var operator = {
  'or$': 'OR',
  'and$': 'AND'
}
...
function parseOperation (entp, sTypes, current_name, current_value, params, values, operator, parseExpression) {
  ...
  for (var i in current_value) {
    ...
    for (var j in w.params) {
      var current_result = parseExpression(entp, sTypes, w.params[j], w.params[i], params, values)
...
function buildQueryFromExpressionCustom (entp, query_parameters, sTypes, values, parseExpression) {
  ...
  if (!_.isEmpty(query_parameters) && query_parameters.params.length > 0) {
    for (var i in query_parameters.params) {
      ...
      if (operator[current_name]) {
        parseOperation(entp, sTypes, current_name, current_value, params, values, operator[current_name], parseExpression)
        continue
      }
...
function savestm (ent, sTypes) {
  ...
  fields.forEach(function (field) {
    escapedFields.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase(field)) + sTypes.escape)
...
function updatestm (ent, sTypes) {
  ...
  fields.forEach(function (field) {
    ...
    if (!_.isUndefined(entp[field])) {
      values.push(entp[field])
      params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase(field)) + sTypes.escape + '=' + RelationalStore.preparedStatements(sTypes.name, ++cnt))
    }
  })
...
function deletestm (qent, q, sTypes) {
  ...
  if (!_.isEmpty(w) && w.params.length > 0) {
    for (var i in w.params) {
      ...
      params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase(param)) + sTypes.escape + '=' + RelationalStore.preparedStatements(sTypes.name, ++cnt))
...
function selectstmOr (qent, q, sTypes) {
  ...
  if (!_.isEmpty(w) && w.params.length > 0) {
    w.params.forEach(function (param) {
      params.push(sTypes.escape + RelationalStore.escapeStr(RelationalStore.camelToSnakeCase('id')) + sTypes.escape + '=' + RelationalStore.preparedStatements(sTypes.name, ++cnt))
    })
